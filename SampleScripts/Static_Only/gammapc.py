...
class Generation:
    def __init__(self, n, popsize, end, items, bpp, moop):
        ...
        self.items = items          # Dictionary of objects to be sorted
        ...
        self.t = 0                  # Generation number counter
        self.p = []
        self.q = []
        self.newgenes = []
    ...
    def rungen(self):
        ...
        self.q, self.newgenes = ga.mutat(self.q, self.newgenes, self.pop,
                                         prob=self.prob_mutat)
        self.fill_q()
    ...
    def add_to_archive(self, solution):
        ...
        if addable:
            if solid in self.archive:
                ...
                self.updateid(1)
                self.archive[solution.getindex()] = solution
            else:
    ...
    def cleanarchive(self):
        ...
        # Every 10 generations check the archive
        if self.t % 10 == 0 or (self.end - self.funkeval) < 500:
    ...
    def initialq(self):
        ...
        for m in range(self.pop):
            p, newsol = self.bpp.generate_newsol(self.idnum, 0, self.prob_ls1,
                                                 self.prob_ls2)
    ...
    def fill_q(self):
        ...
        for m in range(len(self.newgenes)):
            p, newsol = self.bpp.generate_newsol(self.idnum, self.t,
                                                 self.prob_ls1, self.prob_ls2,
                                                 chrom=self.newgenes[m])
            self.updatefe(p - self.idnum)
            self.updateid(p - self.idnum)
            self.q.append(newsol)
    ...
    def update_ideal_values(self):
        ...
        min_maxh = min(self.archive.values(), key=attrgetter('fit1')).fit1
        self.ideal_values[1] = min_maxh
        # Update ideal value for max tready
        min_avgw = min(self.archive.values(), key=attrgetter('fit2')).fit2
        self.ideal_values[2] = min_avgw
    ...
    def update_objective_weights(self):
        ...
        # Update ideal value for avg. initial box heat
        max_maxh = max(self.archive.values(), key=attrgetter('fit1')).fit1
        self.obj_weights[1] = max_maxh
        # Update ideal value for max tready
        max_avgw = max(self.archive.values(), key=attrgetter('fit2')).fit2
        self.obj_weights[2] = max_avgw
        ...
        for p in range(numsol):
            ...
            for q in range(numsol):
                ...
                if u != v:
                    ...
                    if grasp.dom2(v.getfits(), u.getfits()):
                        np += 1
            if np == 0:
                ndset.append(u)
        return ndset
...
def savefitvals(ndset, folder):
    ...
    with open(folder + 'ApproximationSet.csv', 'w') as csvfile:
        ...
        for m in ndset:
            csvwriter.writerow([m.getindex(), m.getbins(), m.getmaxh(),
                                m.getavgw()])


def savexys(ndset, folder):
    # This function saves the x and y matrices into a .h5 file for later
    names = []
    xs = []
    ys = []
    for m in range(len(ndset)):
        index, x, y = ndset[m].getindex(), ndset[m].getx(), ndset[m].gety()
