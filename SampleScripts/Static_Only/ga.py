...
def xover(q, n, prob):
    ...
    for i in range(nover):
        ...
        p = random.random()
        if p <= prob:
            if q[a] not in changed:
                changed.append(q[a])
            if q[b] not in changed:
                changed.append(q[b])
            chroma, chromb = xover_realgene(q[a].getgenes(), q[b].getgenes())
            newgenes.append(chroma)
            newgenes.append(chromb)
...
def mutat(q, newgenes, pop, prob=0.3):
    ...
    #  - prob = 0.3  # default mutation probability
    changed = []
    for m in range(pop):
        p = random.random()
        if p <= prob:
            if m < len(q):
                ...
                newgenes.append(chrom)
            else:
                index = m - len(q)
                chrom = mutat_realgene(newgenes[index])
                newgenes.append(chrom)
